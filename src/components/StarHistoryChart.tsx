...
export function StarHistoryChart({ repos }: StarHistoryChartProps) {
  ...
  useEffect(() => {
    const fetchRepo = (repo: ParsedRepo) => {
      ...
      fetchStarHistory(repo.owner, repo.name)
        .then((data) => {
          setRepoData((prev) => {
            const next = new Map(prev);
            next.set(key, {
  ...
  // Transform data for the chart - show last 52 weeks
  ...
  if (repos.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Weekly Stars</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex h-[300px] items-center justify-center text-muted-foreground">
            Add repositories to see weekly star activity
          </div>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Weekly Stars (Last 52 Weeks)</CardTitle>
        <CardDescription>
          Limited to first 1,000 stargazers per repo due to API pagination
        </CardDescription>
      </CardHeader>
      <CardContent>
        {errors.length > 0 && (
